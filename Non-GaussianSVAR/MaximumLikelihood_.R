...
mle_theta = function(U, l, B0, random_lambda, random_sigma) {
  ...
  options = list(maxit = 20000, reltol = 10^-15)
  ...
  result = optim(par = initial_theta, fn = my_fun4, method = "L-BFGS", control = options, hessian = TRUE)
  ...
  optimal_beta = matrix(optimal_theta[1:9], nrow = nrow(B0), ncol = ncol(B0), byrow = FALSE)
  optimal_beta = maximize_absolute_product_columns(optimal_beta)
...
#TRY TO START FROM THE CHOLEKSY AND TO PERTURBATE THE INITIAL VALUE FOR DIFFERENT TRIES IN ORDER TO GET A KIND OF "GLOBAL OPTIMUM"
lower_bound = -2
upper_bound = 2
...
for (i in 1:500) {
  ...
  varY = VAR(YMC[[50]], p = 2, type = "const")
  residuals_Y = residuals(varY)
  
  mle_result = mle_theta(residuals_Y, l, random_B, random_lambda, random_sigma)
  
  # Store results
  Bthetas4[[i]] = maximize_absolute_product_columns(mle_result$optimal_beta)
...
element_wise_median = apply(simplify2array(Bthetas4), c(1, 2), median)
print(element_wise_median)
