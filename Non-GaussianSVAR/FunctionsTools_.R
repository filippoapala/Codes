...
chi_statBoot = function(boot_B, B_hat){
  ...
  r = numeric(3)
  r = t(matrix(r, nrow  = 1))
  chi = t((R %*% as.vector(B_hat) - r)) %*% solve(cov_B_star_vec) %*% (R %*% as.vector(B_hat) - r) #Get the chi statistics
...
var_coefficients = function(var_model, ts_data) {
  ...
  #Initialize matrix to store VAR coefficients in the required format (+1 for the constant term)
  A_full = matrix(0, nrow = M, ncol = (M * p) + 1)  
  
  coefficients = as.matrix(coef(var_model)) #Extract the coefficients of the VAR model
  
  #Place the lagged coefficients in the appropriate blocks
  for (m in 1:M) {
    
    coef_matrix = coefficients[[m]]  #Each block of lagged coefficients is extracted from the model
...
# ------------------------- USE THE FROBENIOUS NORM TO GET THE CLOSEST VERSION TO B_HAT---------------------------
...
align_bootstrap_sample = function(B_hat, B_boot) {
  ...
  for (perm in perms) {
    ...
    for (signs in 1:nrow(sign_combinations)) {
      ...
      B_boot_perm = B_boot[, perm] * as.numeric(sign_combinations[signs, ]) #Apply permutation and signs
      frob_norm = norm(B_hat - B_boot_perm, type = "F") #Compute Frobenius norm
...
maximize_absolute_product_columns = function(B) {
  ...
  for (i in 1:nrow(column_perms)) {
    ...
    permuted_B = B[, column_perms[i, ]] #Permute the columns
    diagonal = sapply(1:n, function(k) permuted_B[k, k]) #Extract the diagonal elements (row i, column i after reordering)
    current_prod = prod(diag(permuted_B)) #Compute the product
...
Unit_vector_gen = function(M){
  ...
  l = list() #Initialize an empty list to store unit vectors

  #Loop to create each unit vector
  for (i in 1:M) {
    unit_vector = t(rep(0, M)) #Initialize a zero vector of length M
    unit_vector[i] = 1 #Set the i-th element to 1
    l[[i]] = matrix(unit_vector, ncol = 1) #Store the unit vector in the list
  }
