...
generate_moving_block_sample = function(var_model,ts_data, n_lags){
  ...
  block_length = 9  #Adjust block length as needed
  ...
  y_star = matrix(0, nrow = T, ncol = ncol(Residuals))  
  ...
  for (t in (n_lags + 1):(T)) {
    ...
    for (lag in 1:n_lags) {
      lagged_values = as.numeric(y_star[t - lag, ]) #Get the lagged data as a column vector
      coef_block = coef_matrix[, ((lag - 1) * n_vars + 1):(lag * n_vars)] #Extract the corresponding block of coefficients for the current lag
      new_value = new_value + coef_block %*% lagged_values #Update new_value by adding the matrix product
    }
   
    y_star[t, ] =  t(new_value) + bootstrap_residuals_centered[t-n_lags,] + t(const)  #Add the resampled residual
  }
...
generateBootIRFs_ICA = function(n_bootstrap, H, pi, ts_data, var_model2T, bool){
  ...
  for (i in 1:n_bootstrap) {
    ...
    if (bool == 1){
      ...
      B_boot_st = mle_theta(boot_var_residuals, l,  t(chol(cov(residuals(boot_var_model)))),random_lambda, random_sigma )$optimal_beta #choose Cholesky
    }
